    ...
    const fetchFamilyTree = async () => {
      ...
      try {
        ...
        if (data.data.length > 0) {
          ...
          if (
            familyData.name?.toLowerCase().trim() === name.toLowerCase().trim()
          ) {
            setProfilePicture(familyData.picture || defaultProfilePic);
            return;
          }
          ...
          setProfilePicture(matchedPerson?.picture || defaultProfilePic);
        }
      } catch (error) {
        console.error("Error fetching family tree:", error);
      } finally {
      ...
      >
        ...
        <div className="flex py-[1rem] w-full mx-2 md:mx-[1.5rem] items-center justify-between">
          ...
          <button className="md:mr-[2rem] mr-3">
            {collapseBar ? (
              <IoIosArrowDown size={20} className="text-appBrown" />
            ) : (
              <IoIosArrowForward size={20} className="text-appBrown" />
            )}
          </button>
        </div>
      </div>
...
export const FamilyTreeChild = ({
  ...
  useEffect(() => {
    if (openChild && childRef.current && nestedChildRef.current) {
      ...
      if (openChildElement) {
        const { top: childTop } = openChildElement.getBoundingClientRect();
        const { top: nestedChildTop, height: nestedChildHeight } =
          nestedChildRef.current.getBoundingClientRect();
        const { top: parentTop } = childRef.current.getBoundingClientRect();

        const startPosition = childTop - parentTop + 10; // 10px offset for the arrow
        const endPosition = nestedChildTop - parentTop + nestedChildHeight / 2;
            ...
            {descendants.map((child) => (
              ...
              >
                {openChild === child.name &&
                  ...
                  childData[openChild].descendants.length > 0 && (
                    <>
                      {/* L-shaped connector */}
                      <div
                      ...
                      <div
                        ...
                        style={{
                          ...
                          top: `${connectorHeight + 10}px`, // Position it at the end of the vertical line
                        }}
                      />
                ...
                <div className="flex md:ml-[5rem] cursor-pointer ml-[2rem] items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <>
                      ...
                      <div
                        onClick={() => {
                          setSelectedPerson(child.name); // Store selected child's name
                          setShowGallery(true);
                        }}
                        className="cursor-pointer"
                      >
